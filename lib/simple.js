...
`.trim()
    ...
    */
    ...
    async sendGroupInvite(jid, inviteCode, inviteExpiration, groupName = "unknown subject", caption = "Invitation to join my WhatsApp Group", options = {}) {
      let msg = WAMessageProto.Message.fromObject({
        groupInviteMessage: WAMessageProto.GroupInviteMessage.fromObject({
          inviteCode,
          inviteExpiration: parseInt(inviteExpiration) || + new Date(new Date + (3 * 86400000)),
      ...
      let message = await this.prepareMessageFromContent(jid, msg, options)
      await this.relayWAMessage(message)
      return message
    }
...
exports.smsg = (conn, m, hasParent) => {
  ...
  if (m.message) {
    ...
    m.text = (m.mtype == 'buttonsResponseMessage' ? m.msg.selectedButtonId : '') || (m.mtype == 'listResponseMessage' ? m.msg.singleSelectReply.selectedRowId : '') || m.msg.text || m.msg.caption || m.msg || ''
    /**
